...
# ------------
__fzf_select__() {
  command find -L . -type f -print  -o -type d -print  -o -type l -print 2> /dev/null | sed 1d | cut -b3- | fzf -m | while read item; do
    printf '%q ' "$item"
  done
  echo
}
...
__fzf_cd__() {
  local dir
  dir=$(command find -L ${1:-.} -type d -print 2> /dev/null | sed 1d | cut -b3- | $(__fzfcmd) +m) && printf 'cd %q' "$dir"
}
